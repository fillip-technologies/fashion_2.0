...
class HomeController extends Controller
{
    public function homePage()
    {
        ...
        $products = Product::with(['variants', 'category'])->take(4)->orderBy('id', 'desc')->get();
        $top_product = Product::with(['variants', 'category'])
            ->where('show_product', 'top')
            ->take(3)
            ->latest()
            ->get();

        $new_arrival = Product::with(['variants', 'category'])
            ->where('show_product', 'new_arrival')
            ->take(3)
            ->latest()
            ->get();
